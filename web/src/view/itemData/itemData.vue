  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" min-width="120">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateItemDataFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" :title="type==='create'?'添加':'修改'" destroy-on-close>
      <el-scrollbar height="500px">
          <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="外部链接:"  prop="extLink" >
              <el-input v-model="formData.extLink" :clearable="true"  placeholder="请输入外部链接" />
            </el-form-item>
...
const getTableData = async() => {
  const table = await getItemDataList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteItemDataByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateItemDataFunc = async(row) => {
    const res = await findItemData({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.reitemDta
...
// 删除行
const deleteItemDataFunc = async (row) => {
    const res = await deleteItemData({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createItemData(formData.value)
                  break
                case 'update':
                  res = await updateItemData(formData.value)
                  break
                default:
                  res = await createItemData(formData.value)
                  break
